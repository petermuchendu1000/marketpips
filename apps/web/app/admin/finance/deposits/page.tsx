// app/admin/finance/deposits/page.tsx — Deposits console.
    ...
    <div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 text-right">Reconcile</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 && <tr><td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">No deposits match.</td></tr>}
            {rows.map((d: any) => (
              <tr key={d.id} className="border-t hover:bg-muted/30">
                <td className="px-3 py-2 text-xs text-muted-foreground">{d.created_at ? new Date(d.created_at).toLocaleString() : '—'}</td>
                <td className="px-3 py-2">
                  <Link href={`/admin/users/${d.user_id}`} className="text-primary hover:underline">{d.user?.username ?? '—'}</Link>
                  <div className="text-xs text-muted-foreground">{d.phone_number}</div>
                </td>
                <td className="px-3 py-2"><ProviderBadge provider={d.provider} /></td>
                <td className="px-3 py-2 text-right tabular-nums">{Number(d.amount ?? 0).toLocaleString()} {d.currency}</td>
                <td className="px-3 py-2"><TxnStatusBadge status={d.status} /></td>
                <td className="px-3 py-2 text-xs text-muted-foreground">{d.provider_receipt ?? d.failure_reason ?? '—'}</td>
                <td className="px-3 py-2"><DepositActions id={d.id} status={d.status} /></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          {params.page > 1 && <Link href={`/admin/finance/deposits?${qs(params, { page: params.page - 1 })}`} className="rounded-lg border px-3 py-1.5 hover:bg-muted">← Prev</Link>}
          <span className="px-2 py-1.5 text-muted-foreground">Page {params.page} / {totalPages}</span>
          {params.page < totalPages && <Link href={`/admin/finance/deposits?${qs(params, { page: params.page + 1 })}`} className="rounded-lg border px-3 py-1.5 hover:bg-muted">Next →</Link>}
        </div>
      </div>
    </div>
  )
